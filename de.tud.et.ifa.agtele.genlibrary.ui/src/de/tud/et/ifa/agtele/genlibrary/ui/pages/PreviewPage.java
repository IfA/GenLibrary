...
public class PreviewPage extends GenerateGenlibraryEntryPage {
	...
	/* try to generate the library item by launching the eol transformation;
	 * if the transformation has not been successful, generate an error message;
	 * if the transformation has been successfull, populate the tree viewer
	 */
	@Override
	public void setVisible(boolean visible) {
		if(visible) {
			...
			try {
				...
				createLibraryItem(data.geteObject());
			...
			viewer.setInput(data.getLibItemModel());
	 ...
	 */
	private void createLibraryItem(EObject eObject) throws EolUserException, RuntimeException, IOException {
		
		// create a URL representing the eol file in the jar
		URL fileURL = FileLocator.toFileURL(data.getBundle().getEntry("templates/createLibraryItem.eol"));
		
		// create the hashmap containing the parameters to be passed to the eol file
		HashMap<String, Object> params = new HashMap<String, Object>();
		params.put("libraryRoot", eObject); // the root object for the library item
		params.put("fileCopier", new FileCopier()); // the file copier used to copy images
		params.put("version", data.getVersion()); // the version of the library item
		params.put("author", data.getAuthor()); // the author of the library item
		params.put("description", data.getDescription()); // a description for the library item
		params.put("thumbnail", data.getThumbnail()); // the path to a thumbnail image for the 

		// load and execute the eol file
		data.getEolExecutor().executeEol(fileURL, params);
